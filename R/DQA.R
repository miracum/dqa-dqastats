...
DQA <-
  function(target_config,
    ...
    # get configs
    rv$settings_target <-
      getConfig_(config_file = target_config, config_key = rv$db_target)
    rv$settings_source <-
      getConfig_(config_file = source_config, config_key = rv$db_source)
    ...
    # import target SQL
    rv$sql_target <-
      loadSQLs_(utils = rv$utilspath, db = rv$db_target)
    stopifnot(is.list(rv$sql_target))

    # test target_db
    test_target <-
      testTargetDB_(target_settings = rv$settings_target,
                    headless = rv$headless)
    ...
    rv$results_descriptive <-
      descriptiveResults_(rv = rv, headless = rv$headless)

    # get time_interval
    rv$time_interval <-
      timeInterval_(rv$results_descriptive$EpisodeOfCare_period_end)

    # calculate plausibilites
    rv$results_plausibility_atemporal <-
      atempPausiResults_(rv = rv, headless = rv$headless)
    rv$results_plausibility_unique <-
      uniqPausiResults_(
        rv = rv,
    ...
    rv$conformance$value_conformance <-
      valueConformance_(results = rv$results_descriptive,
                        headless = rv$headless)
    value_conformance <-
      valueConformance_(
    ...
    # completeness
    rv$completeness <-
      completeness_(results = rv$results_descriptive,
                    headless = rv$headless)
    ...
    rv$checks$value_conformance <-
      valueConformanceChecks_(results = rv$conformance$value_conformance)
