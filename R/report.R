...
renderValueConformance <- function(results, desc_out, source) {
  cat(paste0(
    "\n- Conformance check: ",
    ifelse(results[[source]]$conformance_error, "failed", "passed"),
    "\n"
  ))

  # get value set
  json_obj <-
    jsonlite::fromJSON(desc_out[[source]]$checks$constraints)
...
renderUniqPlausiRepresentation <- function(pl.item, source) {
  ...
  cat(paste0(
    "\n- Plausibility check: ",
    ifelse(pl.item[[source]]$error == "FALSE", "passed", "failed"),
    "\n"
  ))
...
renderAtempPlausis <-
  function(plausiresults, valueconformance_results) {
    # get names
    obj_names <- names(plausiresults)
