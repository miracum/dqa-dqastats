...
atempPausiResults_ <- function(rv, source_db, headless = FALSE){
  ...
  for (i in names(rv$pl_vars_filter)){
    ...
    if (isFALSE(headless)){
      ...
      progress$set(message = "Getting plausibility descriptions", value = 0)

      # Increment the progress bar, and update the detail text.
      progress$inc(1/length(names(rv$pl_vars_filter)), detail = paste("... working at description of", i, "..."))
    }
    ...
    if (isFALSE(headless)){
      ...
      # Make sure it closes when we exit this reactive, even if there's an error
      on.exit(progress$close())
      progress$set(message = "Calculating plausibility counts", value = 0)

      # Increment the progress bar, and update the detail text.
      progress$inc(1/length(names(rv$pl_vars_filter)), detail = paste("... working at counts of", i, "..."))
    }
    ...
    if (isFALSE(headless)){
      ...
      # Make sure it closes when we exit this reactive, even if there's an error
      on.exit(progress$close())
      progress$set(message = "Calculating plausibility statistics", value = 0)

      # Increment the progress bar, and update the detail text.
      progress$inc(1/length(names(rv$pl_vars_filter)), detail = paste("... working at statistics of", i, "..."))
    }
